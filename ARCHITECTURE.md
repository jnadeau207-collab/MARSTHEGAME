...
- Performance work must pass same-runner base-versus-candidate measurement before replacement.
- Agent-owned production lanes remain accountable to the named human authority.
...
- `tools/performance_baseline.py` records raw samples, median, and MAD for scene setup, update, and offscreen draw work.
- `tools/performance_guard.py` compares base/candidate/base measurements on one runner against `config/performance_thresholds.json` and blocks individual or broad regressions.
- `config/phase0_organization.json` defines the founder-accountable fourteen-seat operating cell; `tools.validate_phase0_organization.py` rejects missing ownership, disciplines, accountability, or truthful headcount.
- GitHub Actions runs Python 3.11 and 3.12 quality gates and uploads replay, organization, baseline, and performance-verdict evidence.
...
The founder holds final product, scope, release, legal-retention, staffing, budget, and merge authority. Fourteen functional seats cover the vertical-slice disciplines required to begin Phase 1. Agent-owned seats can implement and recommend but cannot approve irreversible decisions. The manifest records one human owner and does not misrepresent functional seats as employees.
